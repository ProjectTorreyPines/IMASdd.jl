...
@testset "cocos" begin
    ...
    setproperty!(eqt1d, :psi, [0.1], 11)
    setproperty!(eqt1d, :rho_tor_norm, [1.0], 11)
    @test getfield(eqt1d, :psi) == [0.1]
    @test getproperty(eqt1d, :psi, 11) == [0.1]
    @test getproperty(eqt1d, :psi, 17) == [-0.1]
    @test getfield(eqt1d, :rho_tor_norm) == [1.0]
    @test getproperty(eqt1d, :rho_tor_norm, 11) == [1.0]
    @test getproperty(eqt1d, :rho_tor_norm, 17) == [1.0]

    # user COCOS different from internal_cocos
    setproperty!(eqt1d, :psi, [-0.1], 17)
    setproperty!(eqt1d, :rho_tor_norm, [1.0], 17)
    @test getfield(eqt1d, :psi) == [0.1]
    @test getproperty(eqt1d, :psi, 11) == [0.1]
    @test getproperty(eqt1d, :psi, 17) == [-0.1]
    @test getfield(eqt1d, :rho_tor_norm) == [1.0]
